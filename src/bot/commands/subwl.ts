...
export default function registerSubwl(bot: Telegraf<Context>) {
  bot.command("subwl", async (ctx) => {
    try {
      const text = "text" in ctx.message ? ctx.message.text : "";
      const args = parseArgsSafe(text, 2);
      if (!args) return ctx.reply("Usage: /subwl <clubId> <amount>");
      ...
      // Role + scope check
      const check = canManageClub(ctx, "subwl", clubIdNum); // CAP mapping handled inside canManageClub
