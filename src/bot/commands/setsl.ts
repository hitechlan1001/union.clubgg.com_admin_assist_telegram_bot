...
export default function registerSetsl(bot: Telegraf<Context>) {
  bot.command("setsl", async (ctx) => {
    try {
      const text = "text" in ctx.message ? ctx.message.text : "";
      const args = parseArgsSafe(text, 2);
      if (!args) return ctx.reply("Usage: /setsl <clubId> <amount>");
      ...
      // Role + scope
      const check = canManageClub(ctx, "setsl", clubIdNum);
      ...
      const res = await setLimit(sid, backendId, prevWin, amount, 1);
